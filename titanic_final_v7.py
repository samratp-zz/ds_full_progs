...
def fill_missing_age(missing_age_train, missing_age_test):
    ...
    gbm_reg_grid = model_selection.GridSearchCV(gbm_reg, gbm_reg_param_grid, cv = 10, n_jobs = 10, verbose = 1, scoring = 'r2')
    ...
    print(missing_age_test['Age_GB'])
    
    ada_reg = ensemble.AdaBoostRegressor(random_state = 42)
    ada_reg_param_grid = {'learning_rate': [0.5], 'n_estimators': [2000]}
    ada_reg_grid = model_selection.GridSearchCV(ada_reg, ada_reg_param_grid, cv = 10, n_jobs = 10, verbose = 1, scoring = 'r2')
    ...
    print(missing_age_test['Age_Ada'])
    
    rf_reg = ensemble.RandomForestRegressor(random_state = 42)
    rf_reg_param_grid = {'n_estimators': [2000], 'max_depth': [2, 3], 'max_features': [2]}
    rf_reg_grid = model_selection.GridSearchCV(rf_reg, rf_reg_param_grid, cv = 10, n_jobs = 10, verbose = 1, scoring = 'r2')
    ...
    print(missing_age_test['Age_RF'])
    
    missing_age_test['Age'] = missing_age_test[['Age_GB', 'Age_Ada', 'Age_RF']].mean(axis = 1)
    print(missing_age_test['Age'])
    drop_col_not_req(missing_age_test, ['Age_GB', 'Age_Ada', 'Age_RF'])
...
title_Dict = {}
title_Dict.update(dict.fromkeys(["Capt", "Col", "Major", "Jonkheer", "Don", "Sir", "Dr", "Rev", "the Countess", "Dona", "Lady"], "Other"))
...
missing_age_df = pd.DataFrame(combined_train_test[['PassengerId', 'Age', 'Parch', 'Sex', 'SibSp', 'Family_Size', 'Family_Size_Category', 'Title', 'Fare', 'Fare_Category', 'Pclass']])
...
#### Ticket
combined_train_test['Ticket_Letter'] = combined_train_test['Ticket'].apply(lambda x: str(x)[0])
combined_train_test = pd.get_dummies(combined_train_test, columns = ['Ticket', 'Ticket_Letter'])
combined_train_test.shape

#### Cabin
combined_train_test['Cabin_Letter'] = combined_train_test['Cabin'].apply(lambda x: str(x)[0])
...
voting_params_grid = {'rf__n_estimators': [500], 'rf__min_samples_split': [4], 'rf__max_depth': [10],'rf__max_features':[80], 'rf__n_jobs':[10], 'rf__verbose':[1],
                      ...
                      'xgb__n_estimators': [1000], 'xgb__learning_rate':[0.05], 'xgb__max_depth': [2], 'xgb__min_child_weight':[2]}
