...
def fill_missing_age(missing_age_train, missing_age_test):
    ...
    missing_age_test['Age'] = missing_age_test[['Age_GB', 'Age_Ada', 'Age_RF', 'Age_LRF']].mean(axis = 1).round().astype(int)
...
missing_age_df = pd.DataFrame(combined_train_test[['Age', 'Parch', 'Sex', 'SibSp', 'Family_Size', 'Family_Size_Category', 'Title', 'Fare', 'Fare_Category', 'Pclass']])
missing_age_df = pd.get_dummies(missing_age_df, columns = ['Title', 'Family_Size_Category', 'Fare_Category', 'Sex', 'Pclass'])
...
print(combined_train_test['Age_Category'].groupby(by = combined_train_test['Age_Category']).count().sort_values(ascending = False))
print(combined_train_test.groupby(['Survived', 'Age_Category'])['Survived'].count())
...
combined_train_test.drop(['Name', 'Pclass', 'PassengerId', 'Embarked', 'Sex', 'Title', 'Fare_Category', 'Family_Size_Category', 'Age_Category', 'First_Name', 'Last_Name'], axis = 1, inplace = True)
...
top_n_features = 2000
features_top_n = get_top_n_features(titanic_train_data_X, titanic_train_data_y, top_n_features)
...
voting_est = ensemble.VotingClassifier(estimators = [('rf', rf_est),('gbm', gbm_est),('ada', ada_est),('et', et_est), ('xgb', xgb_est)],
                                       voting = 'soft',
                                       n_jobs = 15)
voting_params_grid = {'rf__n_estimators': [1000], 'rf__min_samples_split': [3], 'rf__max_depth': [20],'rf__max_features':[110], 'rf__n_jobs':[10], 'rf__verbose':[1],
                      'gbm__n_estimators': [1000], 'gbm__learning_rate': [0.05], 'gbm__max_depth': [3], 'gbm__verbose':[1],
                      'ada__n_estimators': [1000], 'ada__learning_rate': [0.5],
                      'et__n_estimators':[1000], 'et__max_depth':[20], 'et__n_jobs':[10], 'et__verbose':[1],
                      'xgb__n_estimators': [1000], 'xgb__learning_rate':[0.05], 'xgb__max_depth': [2]}

voting_grid = model_selection.GridSearchCV(voting_est, voting_params_grid, cv = 10, n_jobs = 10, verbose = 1)
